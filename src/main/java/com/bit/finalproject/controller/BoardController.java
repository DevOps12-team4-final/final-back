...
public class BoardController {
    ...
    public ResponseEntity<?> post(
            // 요청의 멀티파트 데이터중 boardDto라는 이름의 부분을 BoardDto 객체로 변환하여 받는다.
        ...
        try {
            ...
            log.info("post boardDto list : {}", boardDtoList);
    ...
    public ResponseEntity<?> addLike(@PathVariable Long boardId,
                                     @AuthenticationPrincipal CustomUserDetails customUserDetails) {

        Long memberId = customUserDetails.getMember().getUser_id();

        ResponseDto<Object> responseDto = new ResponseDto<>();

        try {
            log.info("addLike boardId : {}", boardId);

            // 좋아요 추가 로직 실행
            boardLikeService.addLike(boardId, memberId);
    ...
    public ResponseEntity<?> removeLike(@PathVariable Long boardId,
                                     @AuthenticationPrincipal CustomUserDetails customUserDetails) {
        ...
        try {
            ...
            // 좋아요 추가 로직 실행
            boardLikeService.removeLike(boardId, memberId);
...
//
//        // 좋아요 정보
//        int likeCount = likeService.getLikeCountByBoardId(boardId);
//
//        // BoardDto에 댓글과 좋아요 추가
