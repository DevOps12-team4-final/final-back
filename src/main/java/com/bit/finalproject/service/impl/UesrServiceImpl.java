...
public class UesrServiceImpl implements UesrService {
    ...
    @Override
    public UesrDto login(UesrDto userDto) {
        ...
        Uesr user = userRepository.findByEmail(userDto.getEmail()).orElseThrow(
        ...
        UesrDto loginUesrDto = user.toDto();

        // 사용자 상태에 따른 로그인 여부
        if(!loginUesrDto.getUserStatus().equals(UserStatus.ACTIVE)){
            throw new RuntimeException("user not active");
        }

        loginUesrDto.setPassword("");
        loginUesrDto.setLastLoginDate(LocalDateTime.now());

        // JWT 토큰 발급
        loginUesrDto.setToken(jwtProvider.createJwt(user));
        System.out.println(loginUesrDto.getToken());
        return loginUesrDto;
    }

    @Override
    public UesrDto join(UesrDto userDto) {
        ...
        UesrDto joinedUesrDto = userRepository.save(userDto.toEntity()).toDto();

        // DB에 저장된 후 joinedUesrDto의 password를 빈 문자열로 설정한다.
        // 보안적인 이유로 password가 클라이언트측에 노출되는 것을 방지하기 위함이다.
        joinedUesrDto.setPassword("");

        return joinedUesrDto;
    }
